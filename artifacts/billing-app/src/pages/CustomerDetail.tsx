...
export default function CustomerDetail() {
  ...
  const [txTypePreset, setTxTypePreset] = useState<"service" | "equipment" | "one_time" | "late_fee" | "payment">("service");
  ...
  // Init edit form when customer loads
  ...
  const handleOpenTxDialog = (type: typeof txTypePreset) => {
    ...
    // Preset descriptions and amounts based on type
    let description = "";
    let amount = 0;
  ...
  // Usually, running balance starts from 0, and we add/subtract sequentially by date.
  // The transactions array from the API is usually newest first. We need to sort by date ascending to calc running balance, then reverse.
  const transactionsWithRunningBalance = customer?.transactions
    ? [...customer.transactions]
        ...
        .reduce((acc, tx) => {
          ...
          acc.push({ ...tx, runningBalance: prevBalance + delta });
          return acc;
        }, [] as any[])
        .reverse()
    : [];
        ...
        <Card className="shadow-sm border-gray-200 bg-gray-50">
          ...
          <CardContent className="flex flex-col items-center justify-center py-6">
            ...
            <p className="text-sm text-gray-500 mt-2">
              {(customer?.balance || 0) > 0 ? "Customer owes you" :
               (customer?.balance || 0) < 0 ? "Customer has a credit" :
               "Account is settled"}
            </p>
          ...
          <Table>
            ...
            <TableBody>
              {isLoading ? (
                ...
                <TableRow>
                  <TableCell colSpan={6} className="text-center py-12 text-gray-500 bg-gray-50/50">
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                              Are you sure you want to delete this {tx.type.replace('_', ' ')} for {formatCurrency(tx.amount)}? This action cannot be undone and will recalculate the customer's current balance.
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
